...
1. Keep the repository structure as-is (notably the new `api/index.py`, `vercel.json`, and root `requirements.txt` files).
2. Provision any required environment variables in Vercel (e.g. `JWT_SECRET`, `DATABASE_URL`).  
   - **Important:** Vercel’s serverless runtime does not provide persistent disk access, so use an external database (PostgreSQL, Neon, Supabase, etc.) instead of the bundled SQLite file.
3. When you deploy, Vercel will install dependencies via `requirements.txt` and mount the Flask app as a serverless function at `api/index.py`.
4. Static pages (e.g. `login.html`, `admin.html`) continue to be served through Flask because all routes map to that function via `vercel.json`.
