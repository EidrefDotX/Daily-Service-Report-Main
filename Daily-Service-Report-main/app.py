...
def create_app() -> Flask:
    ...
    # Basic security headers with performance optimizations
    @app.after_request
    def add_security_headers(resp: Response):  # type: ignore[override]
        resp.headers.setdefault("X-Content-Type-Options", "nosniff")
        resp.headers.setdefault("X-Frame-Options", "SAMEORIGIN")
        resp.headers.setdefault("Referrer-Policy", "strict-origin-when-cross-origin")
        csp = os.getenv("CONTENT_SECURITY_POLICY", "default-src 'self' 'unsafe-inline' data: blob:; connect-src *; img-src 'self' data: blob:; script-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com; style-src 'self' 'unsafe-inline'")
    ...
    def create_engine_with_fallback(url: str):
        ...
        if url.startswith("sqlite"):
            ...
            eng = create_engine(
                url, 
                poolclass=QueuePool,
                pool_size=5,  # Increased from default 5 for better concurrency
                max_overflow=10,  # Allow up to 10 overflow connections
                pool_pre_ping=True,  # Verify connections before using
                pool_timeout=3,  # Reduced from 5s for faster failure detection
                pool_recycle=3600,  # Recycle connections after 1 hour
                connect_args={"connect_timeout": 3}  # Faster connection timeout
            )
    ...
    def auth_login():  # type: ignore[no-untyped-def]
        ...
        db: Session = get_db_session()
        try:
            # Normalize username - optimized path
            if is_admin_user:
